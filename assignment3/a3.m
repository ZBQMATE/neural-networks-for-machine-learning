...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  %				n*256					256*k
  %k = size(data.inputs,2);
  %hid_output = logistic(hid_input);
  %							n*k
  %class_input = model.hid_to_class * hid_output;
  %					10*n 				n*k
  %sumexp = sum(exp(class_input),1);
  %             1*k
  %sumexpmat = (sumexp' * ones(1,10))' .- exp(class_input);
  %										10*k
  %the target output matrix   
  %ret.hid_to_class = data.targets .* (exp(class_input) .* sumexpmat) ./ n_hid ./ (sumexp' * ones(1,10))' ./ (sumexp' * ones(1,10))' + wd_coeffecient*
	%					10*k			10*k
...
function ret = theta_to_model(theta)%theta is a 266n length vector
  ...
  %				266*n_hid
  ret.input_to_hid = transpose(reshape(theta(1: 256*n_hid), 256, n_hid));%first 256n neurons
...
function ret = initial_model(n_hid)
  ...
  % output a vector of [cos1 , cos2, cos3 ... cosn-1]
